...
export function AddTradeModal({ isOpen, onClose, onSubmit, loading = false, error = '' }: AddTradeModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === 'closed' && exitPrice) {
        ...
        tradeData.pnl = action === 'BUY' ? exitValue - entryValue : entryValue - exitValue;
      } else {
        tradeData.pnl = 0;
      }
      ...
      setSubmitError(err instanceof Error ? err.message : 'Failed to add trade');
    }
  };
